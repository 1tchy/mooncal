...
import static logics.SunMoonCalculator.*;

public class PhasesCalculation2 {

	protected double jd_UT = 0;
	protected double t = 0;
	private double obsLon = 0;
	private double obsLat = 0;
	private double TTminusUT = 0;
	private SunMoonCalculator.TWILIGHT twilight = SunMoonCalculator.TWILIGHT.HORIZON_34arcmin;
	protected double slongitude = 0;
	protected double sanomaly = 0;
	 ...
	 * @param s     Second.
	 */
	public PhasesCalculation2(int year, int month, int day, int h, int m, int s) {
		...
		if (jd < 2299160.0 && jd >= 2299150.0) {
			throw new RuntimeException("invalid julian day " + jd + ". This date does not exist.");
		}
	...
	public static SunMoonCalculator factory(LocalDate date, ZoneId offset, double obsLon, double obsLat) {
		ZonedDateTime lastSecondOfDate = ZonedDateTime.of(date, LocalTime.MAX, offset);
		ZonedDateTime lastSecondOfDateInUTC = lastSecondOfDate.withZoneSameInstant(ZoneOffset.UTC);
		final SunMoonCalculator calculation = new SunMoonCalculator(lastSecondOfDateInUTC.getYear(), lastSecondOfDateInUTC.getMonthValue(), lastSecondOfDateInUTC.getDayOfMonth(), lastSecondOfDateInUTC.getHour(), lastSecondOfDateInUTC.getMinute(), lastSecondOfDateInUTC.getSecond(), obsLon, obsLat);
		calculation.calcSunAndMoon();
		return calculation;
	}

	public static LocalDate getNewmoonBefore(LocalDate before) {
		final SunMoonCalculator calculation = factory(before, ZoneOffset.UTC, 0, 0);
		int moonAge = (int) calculation.moonAge;
		return before.minusDays(moonAge);
	}
}
